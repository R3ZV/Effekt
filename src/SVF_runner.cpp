...
auto get_cutoff_sweep_exp(float start_cutoff, float end_cutoff,
                          uint32_t sample_rate, float sweep) -> float {
...
auto main() -> int {
    ...
    std::string params_str = std::format("{:.2f}", resonance_start) + "_" +
                             ...
                             std::format("{:.2f}", start_cutoff_sweep) + "_" +
                             std::format("{:.2f}", end_cutoff_sweep);
